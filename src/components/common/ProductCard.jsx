...
const ProductCard = ({ product }) => {
  ...
  const { id, title, price, description = "", imageUrl, status = "available", ownerId } = product;
  ...
  const handleDelete = async () => {
    try {
      const res = await alertConfirm("Delete product?", `Delete "${title}" permanently? This cannot be undone.`);
      if (!res.isConfirmed) return;

      dispatch(deleteProduct(id));
      alertSuccess("Deleted", `"${title}" has been deleted.`);
    } catch (err) {
      console.error("Delete error:", err);
      alertError("Delete failed", err?.message || "Something went wrong");
    }
  };
  ...
  return (
    <div className="bg-exza-light rounded-xl overflow-hidden shadow-card transition hover:shadow-elevated">
      <div className="relative w-full h-48 bg-gray-100">
        <img src={imgSrc} alt={title} className="w-full h-full object-cover" onError={handleImgError} />
        {status === "sold" && (
          <div className="absolute inset-0 bg-black/60 flex items-center justify-center text-white text-lg font-bold">SOLD</div>
        )}
      </div>

      <div className="p-4 flex flex-col gap-3">
        <div className="flex justify-between items-start">
          <h3 className="text-exza-dark text-base font-semibold truncate">{title}</h3>
        ...
        <p className="text-exza-dark text-sm leading-snug">
          ...
          {description.length > 60 && (
            <button onClick={() => setIsExpanded(!isExpanded)} className="text-blue-600 ml-1 underline">
        ...
        <div className="flex items-center justify-between mt-1">
          <div className="flex gap-3 items-center">
            <button
              ...
              className={`px-3 py-1 rounded-full text-white font-medium transition ${status === "sold" ? "bg-gray-400 cursor-not-allowed" : isInCart ? "bg-green-900 cursor-default" : "bg-exza-purple hover:opacity-90"}`}
            >
            ...
            {isOwner && (
              <>
                <Link to={`/products/edit/${id}`} className="px-3 py-1 rounded-full bg-green-600 text-sm text-white font-medium hover:opacity-90">
                  Edit
                </Link>

                <button onClick={handleDelete} className="px-3 py-1 rounded-full bg-red-400 text-sm text-black font-medium hover:opacity-90">
                  🗑️
                </button>
              </>
            )}
